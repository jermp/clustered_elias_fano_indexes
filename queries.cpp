...
void op_perftest(IndexType& index,
                 ...
                 size_t runs)
{
    ...
    if (false) {
        ...
        logger() << "Mean per query: " << avg / queries.size() << std::endl;
        ...
        stats_line()
            ...
            ("query", query_type)
            ("avg", avg)
            ("avg_per_query", avg / queries.size())
            ("q50", q50)
            ("q90", q90)
            ("q95", q95)
            ;
              ...
              std::string const& query_type)
{
    ...
    for (auto const& t: query_types) {
        ...
        if (t == "and") {
            op_perftest(index, and_query<false>(), queries, type, t, 4);
        } else if (t == "and_freq") {
            op_perftest(index, and_query<true>(), queries, type, t, 4);
        } else if (t == "or") {
            op_perftest(index, or_query<false>(), queries, type, t, 4);
        } else if (t == "or_freq") {
            op_perftest(index, or_query<true>(), queries, type, t, 4);
        } else if (t == "wand" && wand_data_filename) {
            op_perftest(index, wand_query(wdata, 10), queries, type, t, 4);
        } else if (t == "ranked_and" && wand_data_filename) {
            op_perftest(index, ranked_and_query(wdata, 10), queries, type, t, 4);
        } else if (t == "ranked_or" && wand_data_filename) {
            op_perftest(index, ranked_or_query(wdata, 10), queries, type, t, 4);
        } else if (t == "maxscore" && wand_data_filename) {
            op_perftest(index, maxscore_query(wdata, 10), queries, type, t, 4);
        } else {
...
int main(int argc, const char** argv)
{
    ...
    if (type == "clustered_opt") {
        perftest<clustered_opt_index>(index_filename, wand_data_filename,
